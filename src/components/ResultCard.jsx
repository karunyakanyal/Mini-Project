...
export default function ResultCard({ result }) {
  return <section className="card result-card" aria-labelledby="result-heading">
    ...
    <div className="result-grid">
      ...
      <div><h3>Similarity Score</h3><strong className="score">{(result.similarity_score * 100).toFixed(1)}<span>%</span></strong></div>
      <div><h3>Method</h3><p className="method">{result.method}</p></div>
    </div>
    <div className="result-visual"><p>Similarity</p><div className="progress-row"><span>0%</span><SimilarityBar score={result.similarity_score} /><span>100%</span></div></div>
